...
import { LayoutDashboard, CreditCard, Megaphone, MessageSquare, MessagesSquare, LogOut, Bell, Menu, X, User, BookOpen, CalendarCheck, ShieldAlert, FileText, CalendarDays, Video, ShoppingBag, Clock, ListTodo } from 'lucide-react'
...
export default function RepresentativeLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      <aside className={clsx('fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-slate-200 flex flex-col transition-transform duration-200', open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0')}>
          ...
          <div className="relative">
            ...
            {showN && (
              <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden z-50">
                ...
                : notifs.map(n => (<button key={n.id} onClick={() => handleNotif(n)} className="w-full text-left px-4 py-3 hover:bg-slate-50 border-b border-slate-50"><p className="text-sm font-medium text-slate-800">{n.title}</p><p className="text-xs text-slate-500 mt-0.5">{n.body}</p></button>))}
              </div>
            )}
          </div>
        </header>
