...
import { CheckCircle, XCircle, Edit2, Search, ExternalLink, AlertCircle, Clock } from 'lucide-react'
...
export default function AdminPayments() {
  ...
  const approveMut = useMutation({
    ...
    onSuccess: async (_, paymentId) => {
      ...
      if (p) await createNotification({ userId: p.representativeId, schoolId, title: '✅ Pago aprobado', body: `Tu pago de $${p.amountPaid?.toFixed(2)} ha sido aprobado.`, type: 'payment', relatedId: paymentId, read: false })
  ...
  const rejectMut = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      if (p) await createNotification({ userId: p.representativeId, schoolId, title: '❌ Pago rechazado', body: `Tu pago fue rechazado. Motivo: ${rejectReason}`, type: 'payment', relatedId: id, read: false })
  ...
  const editMut = useMutation({
    ...
    onSuccess: () => { toast.success('Monto actualizado'); setShowEditModal(false); qc.invalidateQueries({ queryKey: ['payments'] }) },
  })
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>{['Estudiante','Concepto','Monto','Pagado','Saldo','Estado','Vence','Acciones'].map(h => (
                ...
                {filtered.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-slate-500 text-xs whitespace-nowrap">{p.dueDate ? format(toDate(p.dueDate), 'dd/MM/yyyy', { locale: es }) : '—'}</td>
